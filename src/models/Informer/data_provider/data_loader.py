...
class Dataset_Custom(Dataset):
    def __init__(self, root_path, flag='train', do_pred=False, size=None,
                 features='S', data_path='ETTh1.csv',
                 target='OT', scale=True, timeenc=0, freq='h', train_only=False):
        # size [seq_len, label_len, pred_len]
        # info
    ...
    def __read_data__(self):
        ...
        num_train = max(int(len(df_raw) * (train_split if not self.train_only else 1)), self.seq_len + self.pred_len)
        num_test = max(int(len(df_raw) * test_split), self.seq_len + self.pred_len)
