...
def evaluate_models(X_train, X_val):
    ...
    print("\n" + "="*50)
    print(f"ModelA 各维度MSE: {np.round(mse_A, 4)} | 平均: {mse_A.mean():.4f}")
    print(f"ModelB 各维度MSE: {np.round(mse_B, 4)} | 平均: {mse_B.mean():.4f}")
    print("="*50)
    
    return predA, predB, mse_A.mean(), mse_B.mean()
...
def dynamic_ensemble_predict():
    ...
    # 评估基础模型
    predA, predB, mseA, mseB = evaluate_models(X_train, X_val)
    ...
    choices = selection_probs < 0.5  # 选择概率小于0.5的模型
    choices = choices.astype(np.float32)  # 转换为 float 类型以便后续计算

    # 集成预测
    final_pred = (1-choices) * predA + choices * predB  # 使用广播机制
    mse_ensemble = ((final_pred - X_val)**2).mean()

    mse_final = ((final_pred - X_val)**2).mean(axis=0)
    print(f"集成后的各维度MSE: {np.round(mse_final, 4)} | 平均: {mse_final.mean():.4f}")
    print("="*50 + "\n")
    # 性能对比报告
    print("\n性能对比报告:")
    ...
    print(f"相对最佳模型提升: {improvement:.2f}%")
