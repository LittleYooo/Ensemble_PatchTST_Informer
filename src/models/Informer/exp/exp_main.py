...
class Exp_Main(Exp_Basic):
    ...
    def train(self, setting):
        ...
        #     print('loading model')
        #     self.model.load_state_dict(torch.load(self.args.finetune_weights))
    ...
    def test(self, setting, test=0, data_flag='test'):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                # if (test_data.scale):
                #     preds_inverse_2d = test_data.inverse_transform(pred.reshape(-1, pred.shape[-1]))
                #     trues_inverse_2d = test_data.inverse_transform(true.reshape(-1, pred.shape[-1]))
                #     preds_inverse = preds_inverse_2d.reshape(pred.shape)
                #     trues_inverse = trues_inverse_2d.reshape(pred.shape)
                #     w_dist, w_dist_dims= metric(np.expand_dims(preds_inverse[:,:,:3], axis=0), np.expand_dims(trues_inverse[:,:,:3], axis=0), 1)
                #     w_dists_scaler.append(w_dist)
                # for ii in range(preds_inverse.shape[0]):

                #     # preds_state = Control2State(trues_inverse[ii,:,:], preds_inverse[ii,:,:])
                #     # w_dist, w_dist_dims= metric(np.expand_dims(preds_state[:-1,:3], axis=0), np.expand_dims(trues_inverse[ii,1:,:3], axis=0), 1)
                #     w_dist, w_dist_dims= metric(np.expand_dims(preds_inverse[ii,1:,:3], axis=0), np.expand_dims(trues_inverse[ii,1:,:3], axis=0), 1)
                #     w_dists.append(w_dist)
                #     print(w_dist)
                #     print(w_dist_dims)
                #     # data_plot(preds_state[:-1,:3], trues_inverse[ii,1:,:3], 'figures/'+ str(i) + '.png')
                #     # data_plot(preds_inverse[ii,:,:3], trues_inverse[ii,:,:3], 'figures/'+ str(i) + '_TSTpred.png')
        
        # w_dist_scaler = np.mean(np.concatenate(w_dists_scaler, axis=0))
